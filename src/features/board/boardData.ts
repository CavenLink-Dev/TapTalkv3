// ─── Static board tile data ───────────────────────────────────────────────────
// Extracted from app/(tabs)/talk.tsx (Phase 4 — God-screen split).
// Everything here is module-level constant data: colour palette, tile asset
// map, Fitzgerald word-type colours, the full static tile catalogue, and the
// adapter that converts a persisted CustomBoardTile into a BoardTile.

import type { CustomBoardTile } from '../../context/types';
import type { BoardTile, BoardMode } from './types';
...
// ── Fitzgerald word-type → tile colour ───────────────────────────────────────
// Mirrors the palette in AddSymbolModal so bulk pack imports and single-add
// taps always land with the same colours (Rule 23: colour must stay consistent
// across all entry points). Unknown wordTypes fall back to noun (orange).
